...
4. Discover the metadata the repo offers before setting any: labels
   (`gh label list`); issue types and issue fields, with each field's type,
   id and options:
   `gh api graphql -f query='{repository(owner:"O",name:"R"){issueTypes(first:25){nodes{name}} issueFields(first:25){nodes{__typename ... on IssueFieldSingleSelect{id name options{id name}} ... on IssueFieldMultiSelect{id name options{id name}} ... on IssueFieldNumber{id name} ... on IssueFieldText{id name} ... on IssueFieldDate{id name}}}}}'`;
   the fields of a Project the issue should join
   (`gh project field-list <number> --owner O`, token scope `project`).
   Set what exists; report what does not in one line; create nothing.
5. Create or update the issue in the current repository. Labels:
   `gh issue create --label`, `gh issue edit --add-label`; both take `--type`.
   An issue field is set through `issueFields` on the `createIssue` mutation,
   or `setIssueFieldValue` afterwards; the value key follows the field's
   type (`singleSelectOptionId`, `multiSelectOptionIds`, `numberValue`,
   `textValue`, `dateValue`). A Project field through `gh project item-edit`
   once the issue is in the Project: `gh issue create --project <title>` or
   `gh issue edit --add-project <title>`.
